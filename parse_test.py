...
with ZipFile(zippedfile, mode='r') as zf:
   with open('posts.csv', 'w', newline='') as f:
        ...
        write.writerow(post_fields)
   with open('comments.csv', 'w', newline='') as f:
        write = csv.writer(f, dialect='excel')
        write.writerow(comment_fields)
   with open('media.csv', 'w', newline='') as f:
        write = csv.writer(f, dialect='excel')
        write.writerow(media_fields)
   with open('websites.csv', 'w', newline='') as f:
        write = csv.writer(f, dialect='excel')
        write.writerow(website_fields)
   with open('replies.csv', 'w', newline='') as f:
        write = csv.writer(f, dialect='excel')
        write.writerow(reply_fields)
   ...
   #for message_file in ['files2/a36f5e2adfd045dfa4bccb08a7c58b0c']:
       ...
       with zf.open(message_file, 'r') as f:
           ...
           if len(message_id) > 1 and message_id[0]!='$':
